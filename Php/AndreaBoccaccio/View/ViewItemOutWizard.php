 ...
 * 
 */
class Php_AndreaBoccaccio_View_ViewItemOutWizard extends Php_AndreaBoccaccio_View_ViewConsistentListAbstract {
	...
	private function __construct() {
		$this->setKind('itemOutWizard');
	}

	public static function getInstance() {
		if(self::$instance == null) {
			self::$instance = new Php_AndreaBoccaccio_View_ViewItemOutWizard();
		}
	...
	public function getBody() {
		...
		$selectQuery = 0;
		$querySuccess = FALSE;
		$wizard = Php_AndreaBoccaccio_Model_ItemOutWizard::getInstance();
		...
		if($querySuccess) {
			...
			for($i = 0; $i < $nFields; ++$i) {
				...
				$ret .= "?op=" . $this->getKind() . "&page=0&queryId=" . $queryId . $getWherePrefix . $myGetWhere;
			...
			if($totalPages > 1) {
				...
				$ret .= "<a href=\"" . $_SERVER["PHP_SELF"];
				$ret .= "?op=" . $this->getKind() . "&queryId=" . $queryId . "&page=" . strval(max((intval($actualPage)-1),0)) . $getWherePrefix . $myGetWhere;
				...
				$ret .= "<a href=\"" . $_SERVER["PHP_SELF"];
				$ret .= "?op=" . $this->getKind() . "&queryId=" . $queryId . "&page=" . strval(min((intval($actualPage)+1),(intval($totalPages)-1))) . $getWherePrefix . $myGetWhere;
				...
				$ret .= "<a href=\"" . $_SERVER["PHP_SELF"];
				$ret .= "?op=" . $this->getKind() . "&queryId=" . $queryId . "&page=" . strval((intval($totalPages)-1)) . $getWherePrefix . $myGetWhere;
			...
			$ret .= "?op=" . $this->getKind() . "&queryId=" . $queryId . "&page=0\"> ";
			for($i = 0; $i < $nFields; ++$i) {
